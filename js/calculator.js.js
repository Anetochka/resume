...
function calculateSum(){
  ...
  //Selectors
  const wedsiteTypeSelect = document.querySelector ('#calculator-form-website-type');
  ...
  //Values
  const wedsiteTypeValue = extractPriceFromValue(wedsiteTypeSelect.value);
  ...
  const websiteReceptionValue = convertReseptionOptionToPrice(websiteReception.value);

  console.log(wedsiteTypeValue);
  console.log(technologiesValue);
  console.log(websiteCartValue);
  console.log(websiteReceptionValue);

  
  const totalSum = wedsiteTypeValue + technologiesValue + websiteCartValue + websiteReceptionValue;
  renderSum(totalSum);
} 
